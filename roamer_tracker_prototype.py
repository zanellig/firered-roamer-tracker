...
class StyledMap(KantoMap):
    ...
    def paintEvent(self, _event) -> None:
        ...
        painter.drawPixmap(area, self._map, QRectF(self._map.rect()))
...
class DexVariant(Variant):
    ...
    def _paint_entry(self, painter: QPainter) -> None:
        ...
        painter.setFont(pixel_font(12))
        for index, (name, probability, best) in enumerate(route_chances(self.snapshot)[:4]):
...
class TownMapVariant(Variant):
    ...
    def paintEvent(self, _event) -> None:
        ...
        painter.setPen(self.status_color)
        painter.drawText(
            QRectF(300, 24, 140, 18),
            Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter,
            f"● {self.status}",
        )

        box = QRectF(16, 416, 480, 72)
...
class BattleVariant(Variant):
    ...
    def _player_box(self, painter: QPainter) -> None:
        box = QRectF(216, 506, 296, 58)
        self._slab(painter, box, QColor("#F8F8E8"))
        painter.setFont(pixel_font(10, bold=True))
        gba_text(
            painter,
            QRectF(232, 512, 120, 16),
            Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
            "VOS",
            QColor("#3878C8"),
            QColor("#C8D8F0"),
...
class GlassVariant(Variant):
    ...
    def build_map(self) -> StyledMap:
        widget = GlassMap(self, size=(456, 304))
        widget.move(32, 100)
        return widget
    ...
    def paintEvent(self, _event) -> None:
        ...
        wash.setColorAt(0.0, QColor(18, 22, 34, 190))
        wash.setColorAt(1.0, QColor(8, 10, 18, 225))
        ...
        self._glass(painter, QRectF(16, 84, 488, 336), 26, alpha=30)
    ...
    def _roamer_card(self, painter: QPainter) -> None:
        ...
        title, _hint = headline(self.snapshot)
        ...
        painter.drawText(pill, Qt.AlignmentFlag.AlignCenter, title.title())
...
class SwitcherBar(QFrame):
    ...
    def show_variant(self, variant: Variant) -> None:
        ...
        self.adjustSize()
        self.move((self.host.width() - self.width()) // 2, self.host.height() - self.height() - 10)
        self.raise_()
